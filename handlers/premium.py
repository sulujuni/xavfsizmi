...
async def add_promo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Promokod muvaffaqiyatli yaratildi: `{code}` ({days} kun, turi: {usage_type})",
        parse_mode="Markdown",
    )
...
async def payment_success(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    await update.message.reply_text(
        "🎉 *To'lov muvaffaqiyatli yakunlandi!* Sizga 30 kunlik Premium status taqdim etildi.",
        parse_mode="Markdown",
    )
