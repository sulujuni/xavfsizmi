"""
SafeLink Bot — Phase 2 Main Module.
Async database, caching, rate limiting, and structured logging.
"""
...
# --- Phase 2 Imports ---
from logger import setup_logging, get_logger, metrics, health
from cache_manager import cache
from database import (
    init_db, get_user_lang, set_user_lang, get_group_lang, set_group_lang,
    is_premium, set_premium, get_user_checks, increment_user_checks,
    add_to_history, get_history, add_report, get_stats,
    add_referral, get_referral_count, get_referral_credits, consume_referral_credit,
    get_group_stats, increment_group_blocked, increment_group_warned,
    is_rate_limited, update_rate_limit, redeem_promocode, create_promocode
)
...
from checker import (
    check_url, check_url_with_domain_age, check_url_complete,
...
async def post_init(application: Application):
    """Initialize database, cache, and bot menus on startup."""
    logger.info("Initializing SafeLink Bot (Phase 2)...")

    # Initialize SQLite database
    await init_db()
    health.update("database", True, "SQLite initialized")
    logger.info("Database initialized")

    # Initialize cache (Redis or memory fallback)
    await cache.connect()
    health.update("cache", True, "Connected")
    logger.info("Cache layer initialized")

    # Setup bot menus
    await setup_menu(application)
    logger.info("Bot menus configured")

    await metrics.record_event("bot_start")
    logger.info("SafeLink Bot Phase 2 ready!")
...
async def setup_menu(application: Application):
    """Configure bot menu buttons and commands."""
    public_commands = [
    ...
    try:
        ...
        logger.warning("Menu button setup: %s", e)
    ...
    try:
        ...
        logger.warning("Admin menu setup: %s", e)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle start arguments (Referral or Phishing test)
    if context.args:
        ...
        if args_text.startswith("phish_"):
            ...
            try:
                creator_lang = await get_user_lang(creator_id)
                await context.bot.send_message(chat_id=creator_id, text=t(creator_lang, "phish_alert"), parse_mode="Markdown")
            except Exception:
                pass
            return
            ...
            try:
                ...
                if referrer_id != user.id:
                    success = await add_referral(user.id, referrer_id)
                    if success:
                        try:
                            await context.bot.send_message(
                                chat_id=referrer_id,
                                text="🎉 Yangi do'st taklif qildingiz! Sizga 1 ta bepul /breach balansi qo'shildi."
                            )
                        except Exception:
                            pass
                        await metrics.record_event("referral")
            except Exception:
                pass

    welcome_text = t(lang, "start", name=user.full_name, limit=DAILY_FREE_LIMIT)
    await update.message.reply_text(welcome_text, parse_mode="Markdown")
    await metrics.record_event("new_user")
...
async def language_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        text=t(lang, "choose_language") if not is_group else "🌐 Guruh tilini tanlang / Выберите язык группы:",
...
async def group_language_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await set_group_lang(query.message.chat_id, lang)
...
async def add_promo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await create_promocode(code, days, usage_type)
    await update.message.reply_text(f"✅ Promokod yaratildi: `{code}` ({days} kun, turi: {usage_type})", parse_mode="Markdown")
...
async def promo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success, msg_key = await redeem_promocode(user.id, input_code)
    if success:
...
async def breach_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    lang = await get_user_lang(user.id)
    has_premium = await is_premium(user.id)
    free_credits = await get_referral_credits(user.id)
...
async def breach_receive_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
            ...
            async with session.get(
                ...
                if resp.status == 200:
                    ...
                    if breaches_list:
                        ...
                        result = t(lang, "breach_compromised", email=email, count=len(breaches_list), breaches=breaches_text)
                    else:
        ...
        if not has_premium:
            await consume_referral_credit(user.id)
        await metrics.record_event("breach_check")
    except Exception as e:
...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_report(user.id, url_to_report)
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"📊 *Bot statistikasi:*\nJami foydalanuvchilar: {stats_data['total_users']}\n"
        f"Premium: {stats_data['total_premium']}\nHisobotlar: {stats_data['total_reports']}",
        parse_mode="Markdown"
    )
...
async def payment_success(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    await update.message.reply_text(
        "🎉 *To'lov muvaffaqiyatli!* 30 kunlik Premium status taqdim etildi.",
        parse_mode="Markdown"
    )
    await metrics.record_event("premium_purchase")
    logger.info("Premium purchase: user_id=%d", user_id)
...
# --- PRIVATE MESSAGE HANDLER (URL checking) ---
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_premium(user.id):
        checks = await get_user_checks(user.id)
    ...
    # Run full multi-layer scan
    try:
        report, is_dangerous = await check_url_complete(url, lang)
        status_str = "🔴 Malicious" if is_dangerous else "🟢 Clean"
        await add_to_history(user.id, url, status_str)
        await status_msg.edit_text(text=report, parse_mode="Markdown", disable_web_page_preview=True)
        await metrics.record_event("url_scan")
    except Exception as e:
        logger.error("URL check error for user %d: %s", user.id, e)
...
async def handle_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text("🔍 *APK fayl tahlil qilinmoqda...*", parse_mode="Markdown")
    try:
        ...
        if result.get("success"):
            ...
            await metrics.record_event("apk_scan")
        else:
            await status_msg.edit_text("❌ APK faylni tekshirib bo'lmadi.")
    except Exception as e:
        logger.error("APK error: %s", e)
        await status_msg.edit_text("❌ APK tahlilida xatolik yuz berdi.")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not url:
            await update.message.reply_text("🔍 QR-kod yoki havola topilmadi.")
            return

        status_msg = await update.message.reply_text(
            f"🔗 *QR-kod ichidan havola:* `{url}`\nTahlil qilmoqdaman...",
            parse_mode="Markdown"
        )
        ...
        report = f"🛡 *QR-kod havola hisoboti:*\n\n`{url}`\n\n"
        report += f"Holati: {'🚨 ZARARLI/FISHING' if is_dangerous else '✅ TOZA'}\n"
        report += f"VirusTotal: {vt_res.get('malicious', 0)} ta xavf aniqladi."

        await status_msg.edit_text(report, parse_mode="Markdown")
        await metrics.record_event("qr_scan")
    except Exception as e:
        logger.error("QR error: %s", e)
        await update.message.reply_text("❌ QR-kodni tahlil qilishda xatolik.")
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = await get_group_lang(message.chat_id)
    ...
    for url in urls:
        ...
        if gsb.get("dangerous") or vt.get("malicious", 0) > 0:
            try:
                ...
                await increment_group_blocked(message.chat_id)
            except TelegramError:
...
async def handle_group_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_apk = file_name.lower().endswith(".apk") or mime_type == "application/vnd.android.package-archive"
    if not is_apk:
        return
    ...
    if doc.file_size > 32 * 1024 * 1024:
        await message.reply_text("❌ APK hajmi juda katta (max 32MB).")
        return

    status_msg = await message.reply_text("🔍 *APK tekshirilmoqda...*", parse_mode="Markdown")
    ...
    if not result.get("success"):
        await status_msg.edit_text("❌ APK tekshirib bo'lmadi (timeout).")
        return
    ...
    if mal > 0:
        ...
        await status_msg.edit_text(
            f"⚠️ *Shubhali APK:* `{file_name}`\n{sus}/{total} shubhali.",
            parse_mode="Markdown"
        )
    else:
        await status_msg.edit_text(f"✅ *Xavfsiz APK:* `{file_name}`", parse_mode="Markdown")
...
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await message.reply_text("📸 QR kod aniqlandi, tekshirilmoqda...")
    ...
    try:
        ...
        if is_dangerous:
            ...
            await status_msg.edit_text(f"📸 QR kod URL: `{url}`\n\n✅ Xavfsiz.", parse_mode="Markdown")
    except Exception:
...
def main():
    ...
    logger.info("SafeLink Bot starting with polling...")
    app.run_polling()
